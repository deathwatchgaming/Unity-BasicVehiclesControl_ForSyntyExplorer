/*
*  Name: Jeep 01 Compass
*  File: Jeep01Compass.cs
...
namespace NavigationControl
{
	// public class Jeep01Compass
	public class Jeep01Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The Jeep01 transform")]
			// Transform _jeep01
			[SerializeField] private Transform _jeep01;
		...
		public static Jeep01Compass _jeep01Compass;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Jeep01Compass is enabled
				GetComponent<Jeep01Compass>().enabled = true;

				// Debug Log Jeep01 compass is enabled
			...
			{
				// Debug Log Jeep01 compass is disabled
				...
				// Get Component Jeep01Compass is not enabled
				GetComponent<Jeep01Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _jeep01 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_jeep01.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _jeep01 forward vector

			// Vector3 forward is _jeep01 transform forward
			Vector3 forward = _jeep01.transform.forward;

			// Zero out the Y of _jeep01 forward vector to only get the direction in the X,Z 
	...
	} // close public class Jeep01Compass
