/*
*  Name: Jeep 04 Compass
*  File: Jeep04Compass.cs
...
namespace NavigationControl
{
	// public class Jeep04Compass
	public class Jeep04Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		public static Jeep04Compass _jeep04Compass;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Jeep04Compass is enabled
				GetComponent<Jeep04Compass>().enabled = true;

				// Debug Log Jeep04 compass is enabled
			...
			{
				// Debug Log Jeep04 compass is disabled
				...
				// Get Component Jeep04Compass is not enabled
				GetComponent<Jeep04Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _jeep04 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_jeep04.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _jeep04 forward vector

			// Vector3 forward is _jeep04 transform forward
			Vector3 forward = _jeep04.transform.forward;

			// Zero out the Y of _jeep04 forward vector to only get the direction in the X,Z 
	...
	} // close public class Jeep04Compass
