/*
*  Name: Jeep 02 Compass
*  File: Jeep02Compass.cs
...
namespace NavigationControl
{
	// public class Jeep02Compass
	public class Jeep02Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		public static Jeep02Compass _jeep02Compass;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Jeep02Compass is enabled
				GetComponent<Jeep02Compass>().enabled = true;

				// Debug Log Jeep02 compass is enabled
			...
			{
				// Debug Log Jeep02 compass is disabled
				...
				// Get Component Jeep02Compass is not enabled
				GetComponent<Jeep02Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _jeep02 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_jeep02.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _jeep02 forward vector

			// Vector3 forward is _jeep02 transform forward
			Vector3 forward = _jeep02.transform.forward;

			// Zero out the Y of _jeep02 forward vector to only get the direction in the X,Z 
	...
	} // close public class Jeep02Compass
